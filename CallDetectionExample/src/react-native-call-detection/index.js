...
const requestPermissionsAndroid = async (permissionMessage) => {
      ...
      .then(async (gotPermission) => gotPermission
          ? true
          : await PermissionsAndroid.request(PermissionsAndroid.PERMISSIONS.CALL_PHONE, permissionMessage)
              .then((result) => result === PermissionsAndroid.RESULTS.GRANTED)
        )
}
...
class CallDetectorManager {
    ...
    dispose() {
      console.log(NativeCallDetectorAndroid)
    ...
    hangup() {
      console.log('hangup')
      console.log(NativeCallDetectorAndroid.endCall)
      NativeCallDetector && NativeCallDetector.endCall()
      NativeCallDetectorAndroid && NativeCallDetectorAndroid.endCall()
    }
}
